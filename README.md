...
```sh
cordova plugin add https://github.com/promonteiro89/cordova-plugin-system-bars.git#1.0.4
```
...
- A runtime light/dark **theme switch is honored**: on a `uiMode` change Android resets the bar appearance to the activity theme's default, discarding the active `setStyle`. Because `cordova-android` keeps `uiMode` in the activity's `configChanges`, the activity isn't recreated and the WebView isn't reloaded — so your JS never re-runs to restore it. The plugin re-applies your last `setStyle` on the change, so a runtime `DARK` / `LIGHT` / `DEFAULT` choice survives the toggle (`DEFAULT` re-derives from the new theme).
