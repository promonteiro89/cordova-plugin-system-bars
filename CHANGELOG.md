...
- Native Cordova plugin (Kotlin on Android, Swift 5 on iOS) exposing **strict API parity** with Capacitor 8's [`SystemBars`](https://capacitorjs.com/docs/apis/system-bars) JavaScript surface:
  - `setStyle({ style, bar? })`
...
- On Android, `setAnimation` validates its input to honor Capacitor's contract; the platform composes its own system-bar animation.
